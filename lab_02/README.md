...
+------------------+         HTTP          +------------------+         HTTP          +------------------+
|                  |      (порт 80)        |                  |      (порт 5000)      |                  |
|     КЛИЕНТ       |  ------------------>  |      NGINX        |  ------------------>  |   FLASK API       |
|   (curl/браузер) |       запрос         |  (обратный прокси)|    перенаправление   |  (бизнес-логика)   |
|                  |                      |                  |                       |                  |
+------------------+                      +------------------+                       +------------------+
                                                                                               |
                                                                                               |
...
|-----------|-----------|------------|
| Клиент | curl, браузер | Отправляет HTTP-запросы (GET, POST) |
| Обратный прокси | Nginx | Принимает запросы на порту 80, перенаправляет на Flask |
| Сервер приложений | Python + Flask | Реализует бизнес-логику API |
| Хранилище данных | Список словарей в памяти | Временно хранит информацию о вакансиях |

**Поток обработки запроса:**

1. Клиент отправляет HTTP-запрос на `http://localhost/api/vacancies`
2. Nginx принимает запрос на порту 80 и перенаправляет его на `http://127.0.0.1:5000` в соответствии с правилом `location /api/`
3. Flask-приложение обрабатывает запрос:
   - GET `/api/vacancies` – возвращает список всех вакансий в формате JSON
   - POST `/api/vacancies` – добавляет новую вакансию и возвращает её с присвоенным ID
4. Ответ в формате JSON возвращается клиенту через Nginx

**Преимущества такой архитектуры:**
- Nginx скрывает внутреннюю структуру приложения
- Возможно кеширование ответов на уровне прокси (в данной работе не используется)
- Единая точка входа для всех клиентов
- Простота масштабирования (можно добавить несколько экземпляров Flask за одним прокси)
...
| Файл | Назначение | Где находится |
|------|-----------|----------------|
| `README.md` | Отчёт по лабораторной работе | Корень `lab_02/` |
| `app.py` | Исходный код Flask API | `lab_02/part2_flask_api/app.py` |
| `nginx.conf` | Конфигурация обратного прокси Nginx | `lab_02/part3_nginx_config/nginx.conf` |
| `test_commands.sh` | Скрипт для тестирования API | `lab_02/part2_flask_api/test_commands.sh` |
| `photos/1.png` ... `photos/13.png` | Скриншоты выполнения | `lab_02/photos/` |
